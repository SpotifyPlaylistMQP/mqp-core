...
def get_no_print(mongo_collection):
    indexed_pids = []
    indexed_tids = []
    playlist_dict = {}
    unique_track_dict = {}
...
def post_matrix(matrix, mongo_collection):
    print("posting")
    r = requests.post('http://localhost:8888/mongodb/matrices/' + str(mongo_collection), json={'matrix': matrix})
    print(r.status_code)

def delete_matrix(mongo_collection):
    print("deleting")
    r = requests.delete('http://localhost:8888/mongodb/matrices/' + str(mongo_collection))
    print(r.status_code)
    if r.status_code == 200:
        print("Deleted mongo collection: " + mongo_collection)

def get_matrix(mongo_collection):
    r = requests.get('http://localhost:8888/mongodb/matrices/' + str(mongo_collection))
    if r.status_code == 200:
        return r.json()[0]['matrix']
