...
all_params = {
    "mpd_square_100": {
        ...
        "optimizer_set": ["SGD"],
        "embeddings_set": [0, 1, 2, 3, 4, 5, 100],
        "number_of_playlists_to_test": 100,
        "number_of_runs": 10
    },
    "mpd_square_1000": {
        "alpha_set": [1e-10, 1e8, 1e6, 1e4, 1e2, 1],
        "latent_features_set": [6, 10, 25, 50, 75, 100, 200],
        "learning_rate_set": [1e-2, 1e-4, 1e-6, 1e-8, 1e-10],
...
output.write("alpha, latent_features, learning_rate, percent_zeros, optimizer, embeddings, NDCG\n")
            ...
            for percent_zeros in params["percent_zeros_set"]:
                for optimizer in params["optimizer_set"]:
                    for embeddings in params["embeddings_set"]:
                        ...
                        print("{}, {}, {}, {}, {}, {}, NDCG:{}".format(alpha, latent_features, learning_rate, percent_zeros, optimizer, embeddings, avg_ndcg))
                        output.write("{}, {}, {}, {}, {}, {}, {}\n".format(alpha, latent_features, learning_rate, percent_zeros, optimizer, embeddings, avg_ndcg))
