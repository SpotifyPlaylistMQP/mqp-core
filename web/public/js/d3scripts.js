...
function build_table(data, data2) { //build_table(MF, Feature MF){..}
  ...
  html = html + '<th class="table_index_header">K Value</th>'
  ...
  for (var row in data) {
    if(data[row][' NDCG'] != undefined){
        ...
        var new_row =  ''
        new_row = new_row + '<tr>'
        new_row = new_row + '<td class="table_index_td">'+ (index+1) +'</td>'
        new_row = new_row + '<td class="ndcg_td">'+ Number.parseFloat(data[row][' NDCG']).toPrecision(10) +'</td>'
        new_row = new_row + '<td class="mfndcg_td">'+ Number.parseFloat(data2[row][' NDCG']).toPrecision(10) +'</td>'
        new_row = new_row + '</tr>'
