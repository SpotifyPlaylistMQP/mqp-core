...
# string -> list
def get_unique_songs(input_playlist, playlists):
    tracks = []
    spotify_playlist = playlists[input_playlist]
...
# array -> array, array
split_dictionary = {}
def split_playlist(input_playlist_id, playlists):
    # print("split_playlist called")
    all_tracks = get_unique_songs(input_playlist_id, playlists) #local lists for playlist tracks
    list_80split = [] #80% of the songs
    list_20split = [] #20% of the songs (for removal, non-inclusive)
    spotify_playlist = playlists[input_playlist_id] #Get the given playlist
